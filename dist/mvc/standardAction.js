define(["require", "exports", "olive/components/crossDomainEvent"], function (
  require,
  exports,
  crossDomainEvent_1,
) {
  ...
  class StandardAction {
    ...
    initialize() {
      this.responseProcessor.nothingFoundToProcess.handle((data) =>
        this.runAll(data.response, data.trigger),
      );
    }
      ...
      $("input[name='Startup.Actions']", container).each((index, item) => {
        ...
        if (actions.indexOf(action) === -1) {
          ...
          if (
            ...
            currentUrl != null &&
            currentUrl.contains("/hub/project/")
          ) {
            if (
              stringResult.contains(
                '[{"ServiceKey":"hub","Function":"go","Arguments":["[dashboard]/',
              )
            ) {
    ...
    run(action, trigger) {
      if (action.Notify || action.Notify == "") this.notify(action, trigger);
      else if (action.Script) alert("Script is not supported.");
        ...
        if (opener) {
          ...
          $.post(window.location.href, data, (response) => {
            this.responseProcessor.processAjaxResponse(
              response,
              opener.closest("[data-module]"),
              opener,
              null,
              null,
              null,
        ...
        alert(
          "Don't know how to handle: " + JSON.stringify(action).htmlEncode(),
        );
      return true;
    }
    notify(action, trigger) {
      if (action.AsModal) {
        this.openModalHtmlContent(
    ...
    redirect(action, trigger) {
      if (
        action.Redirect.indexOf("/") != 0 &&
        action.Redirect.indexOf("http") != 0
      )
    ...
    openModalHtmlContent(event, modalTitle, htmlContent, options) {
      ...
      setTimeout(
        () =>
          this.modalHelper.openHtmlContent(
            event,
            modalTitle,
            htmlContent,
            options,
