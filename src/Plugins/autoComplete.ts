...
export default class AutoComplete {
  ...
  enable() {
    ...
    this.valueField = $(
      "[name='" + this.input.attr("name").slice(0, -5) + "']"
    );
  ...
  itemBlured() {
    if (this.valueField.val() == "" && this.input.val() != "") {
      ...
      let suggested = this.input
        .closest(".twitter-typeahead")
        .find(".tt-suggestion");
      let filtered = suggested.filter(
        (e, obj) => obj.innerText === this.input.val()
      );
      ...
      if (filtered.length === 0 && suggested.length === 0) {
        ...
        this.getData(this.input.val(), data => {
          if (data && data.length === 1) {
            ...
            console.warn(
              "There is none or more than one items in the autocomplete data-source to match the given text. Cannot set the value."
            );
          }
        });
      } else {
        // the suggestion list has been displayed
        if (filtered.length === 0) suggested.first().trigger("click");
        else filtered.first().trigger("click");
      }
    }
  }
