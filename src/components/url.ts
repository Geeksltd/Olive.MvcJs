export default class Url implements IService {
    ...
    public goBack(): void {
        if (this.current().indexOf(this.baseContentUrl + "/##") === 0) history.back();
            ...
            const returnUrl = this.getQuery("ReturnUrl");
    ...
    public removeQuery(url: string, parameter: string) {
        ...
        if (urlParts.length >= 2) {
            const prefix = encodeURIComponent(parameter).toLowerCase() + '=';
    ...
    private goToLoginPage() {
        const query: string = this.current().split("/").splice(3).join("/");
    ...
    public addQuery(url: string, key: string, value: any) {
        return url + (url.indexOf("?") === -1 ? "?" : "&") + key + "=" + value;
    }

    public removeEmptyQueries(url: string): string {

        const items = this.fullQueryString(url).trimStart('?').split('&');
        let result = '';

        for (const i in items) {
            const [key, val] = items[i].split('=');
    ...
    public baseContentUrl = (window as any).BaseThemeUrl || '/';
