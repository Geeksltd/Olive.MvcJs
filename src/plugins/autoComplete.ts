        ...
        if (this.valueField.val() == "" && this.input.val() != "") {
            ...
            if (filtered.length === 0 && suggested.length === 0) {
                ...
                this.getData(this.input.val(), data => {
                    if (data && data.length === 1) this.itemSelected(data[0]);
                    else console.warn("There is none or more than one items in the autocomplete data-source to match the given text. Cannot set the value.");
                });
            } else {
